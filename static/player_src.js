const isIos = () => {
    return _iOSDevice = /iPhone|iPod|iPad/.test(navigator.platform);
}
...
const initMP4 = (qualities) => {
    config = BASE_CONFIG
    config.sources = qualities
    config.file = null
    config.type = null
    src_name = 'mp4'
    return config
...
const initDash = (link) => {
    config = BASE_CONFIG
    config.file = link
    config.type = 'application/dash+xml'
    src_name = 'mpd'
    config.sources = null
    return config
}
...
const initPlayer = (data) => {
    src_name = getCookie('source_default')
    console.log(`cookie_default: ${src_name}`)
    if (src_name) {
        config = configSource()
    }
    else {
        config = configSourceIfIos()
    }
...
const configSource = () => {
    if (src_name == 'mpd') {
        config = initDash(data.dash)
    }
    if (src_name == 'mp4') {
        config = initMP4(data.mp4)
    }
    return config
}
...
const changeSourceButton = (player) => {
    if (!isIos()) {
        player.addButton("/static/svg/update-arrow.svg", `Change Source (current: ${src_name.toUpperCase()})`, () => {
            console.log(`source_default: ${src_name}`)
            config = switchSource()
            player.setup(config)
            console.log(`source_changed: ${src_name}`)
            setCookie('source_default', src_name, 365)
            initEvents(player)
        }, "changeSourceButton")
    }
}

const resumePlaybackCookie = (player) => {
    player.on('time', (e) => {
        setCookie(`resumevideodata_${window.video_id}`, e.position, 365);
    })
}

const resumeTime = (player) => {
    position = getCookie(`resumevideodata_${window.video_id}`)
    if (position) {
...
const initEvents = (player) => {
    changeSourceButton(player)
    resumeTime(player)
    resumePlaybackCookie(player)
}

const getInfo = async () => {
    video_id = window.video_id
    const response = await fetch(`${window.location.protocol}//${window.location.host}/public/json/${video_id}`, {
        method: 'POST',
        cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
    })
    json = await response.json()
    if (json.status) {
        data = JSON.parse(localAES.dec(json.data, video_id))
        initPlayer(data)
    } else {
